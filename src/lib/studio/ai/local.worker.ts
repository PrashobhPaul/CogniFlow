...
type InMsg = LoadMsg | GenerateMsg | ProbeMsg | ConfigureMsg | { type: "unload" };
...
env.customCache = {
  async match(request: string) {
    if (request.startsWith(LOCAL_ROOT)) {
      ...
      if (!entry) return undefined; // not vendored → let the library fetch (404 → HF fallback)
      if (entry.chunks.length === 1 && entry.chunks[0] === file) return undefined; // plain file, normal fetch
  ...
  async put(request: string, response: Response) {
    if (request.startsWith(LOCAL_ROOT)) return; // already served from this site
    const cache = await openCache();
    if (!cache) return;
    try {
...
async function generateVision(msg: GenerateMsg): Promise<string> {
  ...
  const inputs: any = await processor(text, [image], { do_image_splitting: false } as any);
  const output: any = await model.generate({
    ...inputs,
    max_new_tokens: msg.maxNewTokens,
    do_sample: false,
...
self.onmessage = async (event: MessageEvent<InMsg>) => {
  ...
  try {
    if (msg.type === "configure") {
      ...
      const device = await resolveDevice(msg.device);
      if (msg.kind === "text") await loadText(msg.modelId, device);
